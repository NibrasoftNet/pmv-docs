...
@Injectable()
export class PmvSubModelMotoSeedService {
  constructor(
  ...
  async run() {
    try {
      const modelCount = await this.repository.count({ where: { pmvCategory: { id: 3 } } });
      if (modelCount === 0) {
        const csvPath = join(__dirname, 'data', 'new-sub-model-moto.csv');
      ...
      console.error('Error in PmvModelSeedMotoService.run:', error);
      throw error;
    }
  }
}