...
export class ArticleSeedService {
  constructor(
    @InjectRepository(IamReferenceEntity)
    private articleRepository: Repository<IamReferenceEntity>,
  ...
  private async processArticles(data: any[], language: LanguageEntity): Promise<IamReferenceEntity[]> {
    const entities: IamReferenceEntity[] = [];
